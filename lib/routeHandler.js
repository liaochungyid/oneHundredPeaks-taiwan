...
handlers.claimsList = function(data, cb) {
  ...
  if (data.method === 'GET') {
    ...
    const page = sanityCheck.number(data.queryStringObject.get('page'),1)
    const offset = sanityCheck.number(data.queryStringObject.get('offset'),10)
    ...
    if (page > 0 && offset > 0) {
      ...
      fileCRUD.fileList('claims', function(err, data) {
        if (!err && data) {
          ...
          date = data.sort((a,b)=> a.createdAt-b.createdAt)
